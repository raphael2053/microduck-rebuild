...
完整清单、货源、供货风险见 **[../hardware/bom.md](../hardware/bom.md)**，此处只放结论。

**合计约 ¥4000–4250**，高于原厂整机 $399（≈¥2900）。自制更贵，价值在学习与可改造性。
**舵机占约 3/4 成本**。

⚠️ **交期是关键路径，舵机先下单**，等货期间做别的阶段。

### 三条已核实的结论（2026-09-16）

**XL330 与 OpenRB-150 在 ROBOTIS 官方渠道均已售罄**，XL330 官网标注补货「11 月中下旬」。

**Seeed Studio 有 XL330 现货且有中国仓**，$30.77/件（10+ 批量价），附舵盘。Seeed 是官方量产伙伴。
⚠️ 其 SKU 标题是「for Reachy Mini」，舵机本体正确但**附带舵盘是否适配鸭子结构件，下单前需确认**。

**U2D2 + Power Hub 不用买**（省 $57）—— ROBOTIS emanual 写明 OpenRB-150 出厂固件即 `usb_to_dynamixel`，完整支持 Dynamixel Wizard 2.0 配 ID 与参数。AI-FanGe 的 BOM 列了 U2D2，但那是多余的，而且它同样缺货。

### 别在第一台上省的

⚠️ **别换飞特舵机。** fanhao375 实测：HD-1910 舵盘凸、XL330 舵盘凹，**8 个配合件要改模重打**，且 AI-FanGe 的代码按 Dynamixel 写。降成本与降风险在这里直接冲突。

**完成判据**：全部物料下单，且舵机与 OpenRB-150 已确认有货。
